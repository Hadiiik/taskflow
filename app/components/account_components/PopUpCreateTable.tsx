...
import ErrorMessagePopUP from "../ErrorMessagePopUP"; // استيراد المكون الجديد
...
const PopUpCreateTable: React.FC<PopUpCreateTableProps> = ({ isSubscription = false, onClose, team_id }) => {
  ...
  const [error, setError] = useState<string | null>(null); // حالة لتتبع الأخطاء
  ...
  const handleCreateTable = async () => {
    if (!tableName.trim()) {
      setError("يرجى إدخال اسم الجدول"); // عرض خطأ إذا كان اسم الجدول فارغًا
      return;
    }
    ...
    try {
      ...
      onClose(); // إغلاق النافذة بعد إنشاء الجدول بنجاح
    } catch {
      setError("فشل في إنشاء الجدول. يرجى المحاولة مرة أخرى."); // عرض خطأ إذا فشل إنشاء الجدول
    } finally {
      setIsLoading(false);
    }
  };
  ...
  return (
    <>
        ...
        <div ref={popupRef} className="bg-white rounded-lg shadow-lg w-full max-w-sm p-5 relative">
          ...
          <div className="mt-3">
            ...
            <input
              ...
              required // إضافة required لحقل اسم الجدول
            />
          </div>
          ...
          <div className="mt-4 flex flex-col gap-2">
            ...
            <button
              ...
              onClick={handleCreateTable} // استدعاء handleCreateTable مباشرة
            >
              إنشاء الجدول
            </button>
          </div>
      ...
      {error && <ErrorMessagePopUP message={error} onClose={() => setError(null)} />} {/* عرض نافذة الخطأ */}
    </>
  );
};
